...
function shuffleOptions(question) {
  ...
  for (const n of oldNums) {
    if (answerText.startsWith(n)) {
      answerContent = answerText.slice(n.length).trim();
      break;
    }
  }
  const newOptions = options.map((opt, i) => {
    let content = opt;
    for (const n of oldNums) {
      if (opt.startsWith(n)) { content = opt.slice(n.length).trim(); break; }
    }
...
export default function StudyQuizApp() {
  ...
  const buildPrompt = () => {
    ...
    let typeGuide = "";
    if (subject === "영어") {
      if (englishType === "multiple") {
        typeGuide = "모든 문제를 객관식(5지선다)으로 출제하세요. 빈칸추론, 내용일치, 어법/어휘 유형을 포함하세요.";
      } else if (englishType === "essay") {
        typeGuide = `모든 문제를 서술형으로 출제하세요. 
한글 문장을 보여주고 영어로 영작하는 문제를 위주로 출제하세요.
예시) question: "다음 한글을 영어로 쓰시오: '나는 매일 아침 학교에 걸어간다.'" type: "서술형" answer: "I walk to school every morning."
영작 외에 단답형(단어/표현 쓰기)도 포함하세요.`;
      } else {
        typeGuide = "객관식(빈칸/일치/어법)과 서술형(영작/단답)을 골고루 섞어서 출제하세요.";
      }
    }
  ...
  "questions": [
    {
      ...
      "answer": "③ 내용",
      "explanation": "③이 정답인 이유. ①은 ~때문에 틀림. ②는 ~때문에 틀림."
    },
    {
      "id": 2,
      "type": "서술형",
...
채점 결과를 JSON으로만 응답하세요:
{
  ...
  "feedback": "간단한 피드백 (1-2문장)"
      ...
      const data = await response.json();
      if (data.result) {
        setEssayScores(prev => ({ ...prev, [question.id]: data }));
      }
    } catch (err) {
  ...
  const reset = () => {
    ...
    setSelectedAnswers({}); setShowAnswers(false); setScore(null); setError(null); setEssayScores({});
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
            ...
            {uploadedImages.length > 0 && (
              <div style={{ background:"rgba(255,255,255,0.05)", borderRadius:"16px", padding:"16px", border:"1px solid rgba(167,139,250,0.2)" }}>
                ...
                <button onClick={() => setStep("config")} style={{ width:"100%", marginTop:"14px", padding:"14px", borderRadius:"14px", border:"none", background:"linear-gradient(135deg,#7c3aed,#a855f7)", color:"#fff", fontSize:"15px", fontWeight:"800", cursor:"pointer" }}>다음 단계 →</button>
              </div>
            )}
                      ...
                      {q.options.map((opt,oi) => {
                        ...
                        return (
                          <button key={oi} onClick={()=>handleSelectAnswer(q.id,opt)} style={{ padding:"12px 16px", borderRadius:"10px", border:`2px solid ${isAnswer?"#4ade80":isMyWrong?"#f43f5e":isSelected?"#a78bfa":"rgba(255,255,255,0.1)"}`, background:isAnswer?"rgba(74,222,128,0.12)":isMyWrong?"rgba(244,63,94,0.1)":isSelected?"rgba(167,139,250,0.15)":"rgba(255,255,255,0.03)", color:isAnswer?"#4ade80":isMyWrong?"#f43f5e":isSelected?"#a78bfa":"#cbd5e1", textAlign:"left", cursor:showAnswers?"default":"pointer", fontSize:"14px", fontWeight:isSelected||isAnswer?"700":"400", display:"flex", alignItems:"center", gap:"8px" }}>
                            ...
                            {isAnswer && showAnswers && <span style={{ fontSize:"12px", flexShrink:0 }}>✓ 정답</span>}
                            {isMyWrong && <span style={{ fontSize:"12px", flexShrink:0 }}>← 내 선택</span>}
                          </button>
                        );
                      })}
